...
class LockVersionCommand implements ICommand
{
	...
	public function run(args:Array<String>):Void
	{
		...
		var versionText = successes.length == 1 ? "library version has been updated" : "library versions have been updated";
...
';
	...
	{
		if (!FileSystem.exists('.haxelib/${periodsToComma(name)}/hg/.hg'))
			throw new ValidationError('Library $name is not checked out', 1);
	...
	function getHgRef(name:String)
	{
		...
		if (!~/^[a-z0-9]+$/i.match(newRef))
			throw new ValidationError('Library $name has no ref', 1);
		if (newRef.endsWith("+"))
			newRef = newRef.substr(0, newRef.length - 1); // changes marker
		return newRef;
	}

	function lockGitVersion(name:String, url:String, ref:Option<String>, dir:Option<String>, longId:Bool)
	{
		if (!FileSystem.exists('.haxelib/${periodsToComma(name)}/git/.git'))
			throw new ValidationError('Library $name is not checked out', 1);
	...
	function lockHaxelibVersion(name:String, version:Option<String>)
	{
		...
		if (!result.isInstalled)
			throw new ValidationError('Library $name is not installed', 1);
